...
- **`workflow/nodes/openai_image_node.py`**: OpenAI Image API — `images.generate` (default) or `images.edit` when `input_images_template` resolves to path(s); model default **`gpt-image-2`**; **`OPENAI_API_KEY`**; paths as **`{run_id}/images/<node_id>_<n>.png`** for `/artifacts`. Node type id remains **`media.gemini_image`** for stored workflows.
